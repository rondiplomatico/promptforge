#!/bin/bash
# claudeloop installer — interactive, no CLI arguments
# Installs hooks, commands, and skills to a target .claude/ directory
set -e

REPO_DIR="$(cd "$(dirname "$0")" && pwd)"

echo "=== claudeloop installer ==="
...
# Create directories
mkdir -p "$TARGET_CLAUDE_DIR/claudeloop/logs"
echo "  Created  $TARGET_CLAUDE_DIR/claudeloop/logs/"
mkdir -p "$TARGET_CLAUDE_DIR/claudeloop/hooks"

# Install hook scripts
chmod +x "$REPO_DIR/hooks/"*.sh
for hook in "$REPO_DIR/hooks/"*.sh; do
  install_file "$hook" "$TARGET_CLAUDE_DIR/claudeloop/hooks/$(basename "$hook")"
done

# Always copy schema (small file)
cp "$REPO_DIR/schema.json" "$TARGET_CLAUDE_DIR/claudeloop/schema.json"
echo "  Copied   $TARGET_CLAUDE_DIR/claudeloop/schema.json"

# Install commands
mkdir -p "$TARGET_CLAUDE_DIR/commands/claudeloop"
for cmd in "$REPO_DIR/commands/"*.md; do
  [ -f "$cmd" ] || continue
  install_file "$cmd" "$TARGET_CLAUDE_DIR/commands/claudeloop/$(basename "$cmd")"
done
...
for skill_dir in "$REPO_DIR/skills/"*/; do
  ...
  install_dir "$skill_dir" "$TARGET_CLAUDE_DIR/skills/claudeloop-${skill_name}"
done
...
HOOKS_DIR="$TARGET_CLAUDE_DIR/claudeloop/hooks"

# Build the hooks array using jq
# Read existing settings, remove old claudeloop hooks, add new ones
UPDATED=$(jq --arg hdir "$HOOKS_DIR" '
  # Remove existing claudeloop hook entries (by command path containing "claudeloop")
  .hooks = ((.hooks // []) | map(select(.command | tostring | contains("claudeloop") | not)))
  # Also remove old log-prompts.sh hook
  | .hooks = (.hooks | map(select(.command | tostring | contains("log-prompts.sh") | not)))
  # Add new claudeloop hooks
  | .hooks += [
    {
...
echo "[3] Import existing session history into claudeloop logs?"
...
if [ "$IMPORT_CHOICE" = "y" ]; then
  if ! command -v python3 >/dev/null 2>&1; then
    ...
    python3 "$REPO_DIR/scripts/extract-sessions.py" \
      --include-old-logs \
      --output "$TARGET_CLAUDE_DIR/claudeloop/logs/" \
