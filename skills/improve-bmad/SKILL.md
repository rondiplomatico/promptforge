...
1. **Check Scope**:
   BMAD configuration (`.bmad-core/`) is project-local. If global scope was selected, inform the user and offer to switch to project scope or abort.

2. **Check for Friction Report**:
   Look for `$SCOPE_FRICTION_REPORT`. If it doesn't exist, inform the user to run `/promptforge:analyze-corrections` first.

3. **Identify BMAD-related friction**:
...
4. **Gather BMAD context via task agent**:
